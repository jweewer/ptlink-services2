...
int listnicks_stdb = 0; /* generate sdtb file flag */
int listchans_stdb = 0; /* generate sdtb file flag */
/*************************************************************************/
...
void do_listnicks(int ac, char **av)
{
	...
	if (av[i][0] == '-') {
	    ...
	    case 'e':
		if (av[i+1])
		    expiremail = atol(&av[i+1][0])*24*3600;		    		    
		    ac--;
		break;
	    case 's':
		if (i > 1)
	...
	fprintf(stderr, "\
\n\
Usage: listnicks [-c] [-d data-dir] [-e n] [-s] [nick [nick...]]\n\
     -c: display only count of registered nicks\n\
            (cannot be combined with nicks)\n\
     -e: display email and nick from nicks expiring in n days from now\n\
     -s: generate nicks.stdb files (to be used with import-stdb)\n\
...
void do_listchans(int ac, char **av)
{
	...
	fprintf(stderr, "\
\n\
Usage: listchans [-c] [-s] [-d data-dir] [channel [channel...]]\n\
