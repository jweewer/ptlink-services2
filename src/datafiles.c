...
#ifndef NOT_MAIN
	log1("Error reading version number on %s: End of file detected",
		f->filename);
#endif
	return 0;
    } else if (version > FILE_VERSION || version < -1) {
#ifndef NOT_MAIN
	log1("Invalid version number (%d) on %s", version, f->filename);
#endif
	return 0;
    }
    return version;
...
static dbFILE *open_db_write(const char *service, const char *filename)
{
    ...
    *f->backupname = 0;
	...
	int errno_save = errno;
#ifndef NOT_MAIN
	log1("Opening %s database %s for write: Filename too long",
		service, filename);
#endif
	free(f);
	errno = errno_save;
...
#ifndef NOT_MAIN
    if(!f->fp) wallops(NULL,"File could not be created");
#endif    
...
int read_string_buffer(char *ret, dbFILE *f)
{
    uint16 len;
...
/* */
char* m_read_string (FILE * f, char *filename)
{
    char *s;
    int len;
